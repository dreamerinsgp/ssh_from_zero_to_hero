...
/// Returns negotiated algorithms, session keys, session_id, and the stream
pub fn perform_key_exchange(
    mut stream: TcpStream,
    client_version: &VersionString,
    server_version: &VersionString,
) -> Result<(NegotiatedAlgorithms, SessionKeys, Vec<u8>, TcpStream)> {
...
fn perform_dh_key_exchange(
    ...
    let server_newkeys_payload = read_packet(&mut stream)
    ...
    Ok((negotiated.clone(), session_keys, session_id, stream))
}
