...
// Returns a Result containing a tuple of (server_version, client_version) on success
pub fn perform_version_exchange(mut stream: TcpStream) -> Result<(VersionString, VersionString)> {
    // Server sends its version string immediately after TCP connection
    // Create a new VersionString instance for the server
    // Parameters: protocol version "2.0", software name "ssh_stage2_version_exchange", optional comments None
    let server_version = VersionString::new("2.0", "ssh_stage2_version_exchange", None);
    ...
    // Return success with a tuple containing both version strings
    // Ok() wraps the tuple in a Result::Ok variant
    // The tuple (server_version, client_version) contains both VersionString instances
    // This allows the caller to know both what was sent and what was received
